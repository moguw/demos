...
@allure.step("test_resetStudentUserPassword")
# @pytest.mark.run(order=3)
@graphql_control.get_graphql_query()
@pytest.mark.parametrize("Args",Yamls()["test_resetStudentUserPassword"])
def test_resetStudentUserPassword(authToken,Args,query=None):
    ...
    if handle_db_data(reset_password_token_sql)["reset_password_token"] is not None:
        ...
        logger.info("%s user's password has been reseted" %user_id)